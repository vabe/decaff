...
size_t CIFF::createHeader(string magic, int hs, int cs, vector<string> rawCiff, size_t curr_pos)
{
    try
    {
        ...
        if (header.content_size != header.width * header.height * 3)
            throw CONTENT_SIZE_NOT_WXHX3_ERROR;
        ...
        if (header.header_size != header.magic.length() + 8 + 8 + 8 + 8 + header.caption.length() + 1 + tagsLength + 3)
            throw CIFF_HEADER_SIZE_ERROR;
    }
...
void CIFF::readContent(vector<string> rawCiff, size_t curr_pos)
{
    ...
    try
    {
        for (size_t i = curr_pos; i < curr_pos + header.content_size; i++)
        {
            ...
            if (HexToInt(rawCiff[i]) > 255 || HexToInt(rawCiff[i]) < 0)
                throw RGB_NOT_IN_RANGE_ERROR;
        ...
        if (content.size() != header.content_size / 3)
            throw CONTENT_SIZE_ERROR;
    }
    catch (CiffStatus errorStatus)
    {
        setStatus(errorStatus);
