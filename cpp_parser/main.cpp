...
int main(int argc, char *argv[]) {
    ...
    logger->SetLogPreferences(path +"log/"+outputName+".txt", logger->GetLogLevel("INFO"), logger->GetLogOutput("FILE"));
    logger->Log(__FILE__, __LINE__, "INIT Filename: " + current_file_name, LogLevel::INFO);


    //_CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF);
    ifstream input( path + "caff_files/" + current_file_name, std::ios::binary );
